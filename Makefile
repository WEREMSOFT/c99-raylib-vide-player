...
EMSC_CFLAGS := -O2 -s -Wall -std=c99 -D_DEFAULT_SOURCE -Wno-missing-braces -s DISABLE_DEPRECATED_FIND_EVENT_TARGET_BEHAVIOR=0 -s USE_GLFW=3 -s TOTAL_MEMORY=67108864 -v -D OS_WEB --preload-file assets -lopenal
EMSC_CC := emcc
...
$(HTML_D)%.html: $(SRC_FILES)
	$(EMSC_CC_COMMAND) -g4 --source-map-base https://weremsoft.github.io/c99-raylib-vide-player/ $^ -o $@ $(EMSC_STATIC_LIBS_D)
	cp -r src $(HTML_D)/src
